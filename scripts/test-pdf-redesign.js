...
const { reportGenerationService } = require('../dist/services/reportGeneration');
const fs = require('fs');
 ...
 */
function generateMockData(tagName, startTime, endTime, pointCount = 100) {
  const data = [];
 ...
 */
async function testSingleTagReport() {
 ...
 */
async function testMultipleTagsReport() {
 ...
 */
async function testFooterAndMetadata() {
  ...
  try {
    ...
    if (result.success) {
      ...
      console.log(`     - "Generated by OT Hub on ${generatedAt.toLocaleString()}"`);
 ...
 */
async function testPrintReadability() {
 ...
 */
async function testPageCountOptimization() {
...
function printTestSummary(results) {
  ...
  console.log('     - "OT Hub" subtitle (default app name) in medium gray (#6b7280)');
  console.log('     - Horizontal line separator below header');
  console.log('\n□ 2. Footer on All Pages:');
  console.log('     - Every page has a footer');
  console.log('     - Footer includes "Generated by OT Hub on [timestamp]"');
