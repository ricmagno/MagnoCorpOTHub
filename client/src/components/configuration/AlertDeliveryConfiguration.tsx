...
export const AlertDeliveryConfiguration: React.FC = () => {
  ...
  const loadConfig = useCallback(async () => {
    try {
      ...
      if (data.success && data.data) {
        setConfig({ ...DEFAULT_CONFIG, ...data.data, smtpPassword: '' });
      }
    } catch {
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        success('Test email sent', `Email sent to ${testRecipient}`);
      } else {
        toastError('Test failed', data.message || 'Could not send test email');
      }
    } catch (err: any) {
    ...
    <div className="space-y-6">
      ...
      {activeTab === 'email' ? (
            ...
            <form onSubmit={handleSave} className="space-y-6">
              ...
              <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">
                    Send alert notifications and scheduled reports via email
                  </p>
                </div>
                <button
                  type="button"
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="md:col-span-2">
                    {textField('Host', 'smtpHost', 'text', 'smtp.gmail.com')}
                  </div>
                  <div>
                    {textField('Port', 'smtpPort', 'number', '587')}
                  </div>
                </div>
              ...
              <div>
                ...
                <p className="mt-2 text-xs text-gray-500">
                  If left blank, the username above is used as the sender address.
                </p>
              </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
          ...
          <h3 className="text-lg font-semibold text-gray-700 mb-2">SMS Alerts</h3>
          <p className="text-sm text-gray-500 max-w-sm mx-auto">
            SMS alert delivery configuration will be available in a future update.
          </p>
        </div>
      )}
    </div>
