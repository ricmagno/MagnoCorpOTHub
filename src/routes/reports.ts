...
const reportConfigSchema = reportConfigBaseSchema.refine(
  ...
  (data) => {
    if (data.specificationLimits) {
      const validation = validateSpecificationLimitsMap(data.specificationLimits);
      if (!validation.isValid) return { message: validation.errors.join('; '), path: ['specificationLimits'] };
    }
...
router.post('/generate', authenticateToken, requirePermission('reports', 'write'), asyncHandler(async (req: Request, res: Response) => {
  ...
  apiLogger.info('Starting end-to-end report generation', { reportId, tags: config.tags, timeRange: config.timeRange });
  ...
  try {
    ...
    if (!validation.valid) {
      apiLogger.error('Data flow configuration validation failed', { reportId, errors: validation.errors });
    ...
    const result = await dataFlowService.executeDataFlow({
      ...
      preGeneratedCharts: req.body.charts ? Object.entries(req.body.charts as Record<string, string>).reduce((acc, [name, base64]) => {
        if (base64 && base64.startsWith('data:image/')) {
          ...
          if (parts.length > 1 && parts[1]) acc[name] = Buffer.from(parts[1], 'base64');
        }
        return acc;
    ...
    if (!result.success) {
      apiLogger.error('Report generation failed', { reportId, error: result.error, dataMetrics: result.dataMetrics });
      throw createError(result.error || 'Report generation failed', 500);
    }

    apiLogger.info('Report generation completed successfully', { reportId, dataMetrics: result.dataMetrics, reportMetadata: result.reportResult?.metadata });

    res.json({
      success: true, reportId, status: 'generated',
      message: 'Report generation completed successfully',
      config: reportConfig, generatedAt: new Date().toISOString(),
      downloadUrl: `/api/reports/${reportId}/download`,
      metadata: result.reportResult?.metadata, dataMetrics: result.dataMetrics
    });

  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    apiLogger.error('Report generation failed with exception', { reportId, error: errorMessage });
    res.status(500).json({
      success: false, reportId, status: 'failed',
      message: 'Report generation failed', error: errorMessage,
...
router.get('/', authenticateToken, requirePermission('reports', 'read'), asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    const reports = await reportManagementService.listReports(userId as string || currentUserId);
    let filteredReports = reports;
    if (search) {
      const searchTerm = (search as string).toLowerCase();
      filteredReports = reports.filter(report =>
        report.name.toLowerCase().includes(searchTerm) || report.description?.toLowerCase().includes(searchTerm)
      );
    }
    const startIndex = (Number(page) - 1) * Number(limit);
    const paginatedReports = filteredReports.slice(startIndex, startIndex + Number(limit));
    res.json({
      success: true, data: paginatedReports,
      pagination: { page: Number(page), limit: Number(limit), total: filteredReports.length, pages: Math.ceil(filteredReports.length / Number(limit)) }
    });
  } catch (error) {
...
router.post('/save', authenticateToken, requirePermission('reports', 'write'), asyncHandler(async (req: Request, res: Response) => {
  ...
  apiLogger.info('Saving report configuration', { name: saveRequest.name, userId: currentUserId });
  try {
    const result = await reportManagementService.saveReport(saveRequest, currentUserId);
    if (result.success) {
      res.status(201).json({ success: true, data: { reportId: result.reportId, version: result.version, name: saveRequest.name, description: saveRequest.description }, message: result.message });
    } else {
...
router.post('/', authenticateToken, requirePermission('reports', 'write'), asyncHandler(async (req: Request, res: Response) => {
  ...
  const saveRequest = requestResult.data as any;
  const currentUserId = (req as any).user?.id || 'anonymous';
  apiLogger.info('Saving report configuration (legacy endpoint)', { name: saveRequest.name, userId: currentUserId });
  try {
    const result = await reportManagementService.saveReport(saveRequest, currentUserId);
    if (result.success) {
      res.status(201).json({ success: true, data: { reportId: result.reportId, version: result.version, name: saveRequest.name, description: saveRequest.description }, message: result.message });
...
router.get('/:id', authenticateToken, requirePermission('reports', 'read'), asyncHandler(async (req: Request, res: Response) => {
  initializeServices();
  const { id } = req.params;
  if (!id) throw createError('Report ID is required', 400);
...
router.get('/:id/versions', authenticateToken, requirePermission('reports', 'read'), asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!versionHistory) throw createError('Report not found', 404);
    const fullHistory = await reportManagementService.getReportVersions(id);
    res.json({ success: true, data: { ...versionHistory, versions: fullHistory?.versions || [] } });
  } catch (error) {
    if (error instanceof Error && error.message.includes('not found')) throw error;
...
router.post('/:id/versions', authenticateToken, requirePermission('reports', 'write'), asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    const newVersion = await reportManagementService.createNewVersion(id, config as any, currentUserId, changeDescription);
    if (!newVersion) throw createError('Failed to create new version', 500);
    res.status(201).json({ success: true, data: newVersion, message: `New version ${newVersion.version} created successfully` });
  } catch (error) {
...
router.put('/:id', authenticateToken, requirePermission('reports', 'write'), asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!configResult.success) throw createError('Invalid report configuration', 400);
  ...
  try {
    ...
    if (!existingReport) throw createError('Report not found', 404);
    const updatedConfig = {
      ...existingReport.config, ...updates,
    ...
    const newVersion = await reportManagementService.createNewVersion(existingReport.name, updatedConfig, currentUserId, req.body.changeDescription || 'Updated configuration');
    if (!newVersion) throw createError('Failed to update report', 500);
    res.json({ success: true, data: { id: newVersion.id, version: newVersion.version, config: newVersion.config }, message: 'Report configuration updated successfully' });
  } catch (error) {
    if (error instanceof Error && error.message.includes('not found')) throw error;
...
router.delete('/:id', authenticateToken, requirePermission('reports', 'delete'), asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (success) {
      res.json({ success: true, message: 'Report configuration deleted successfully' });
    } else {
...
router.post('/export', authenticateToken, requirePermission('reports', 'read'), asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!config) { apiLogger.error('Export failed: missing config'); throw createError('Report configuration is required', 400); }
  if (!format || !isValidExportFormat(format)) { apiLogger.error('Export failed: invalid format', { format }); throw createError('Invalid export format. Must be "json" or "powerbi"', 400); }
  apiLogger.info('Starting report configuration export', { format, configName: config.name, tags: config.tags?.length || 0 });
  try {
    const result = await configExportService.exportConfiguration(config, { format, includeMetadata: true });
    res.setHeader('Content-Type', result.contentType);
    res.setHeader('Content-Disposition', `attachment; filename="${result.filename}"`);
    res.setHeader('Cache-Control', 'no-cache');
    res.send(result.data);
    apiLogger.info('Export completed successfully', { format, filename: result.filename, size: Buffer.isBuffer(result.data) ? result.data.length : Buffer.byteLength(result.data, 'utf8') });
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    apiLogger.error('Export failed', { format, configName: config.name, error: errorMessage });
    if (errorMessage.includes('exceeds maximum export size')) throw createError(errorMessage, 413);
    else if (errorMessage.includes('Unsupported export format')) throw createError(errorMessage, 400);
    else throw createError(`Failed to export configuration: ${errorMessage}`, 500);
  }
}));
...
router.post('/import', authenticateToken, requirePermission('reports', 'write'), asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!fileContent || typeof fileContent !== 'string') { apiLogger.error('Import failed: missing or invalid fileContent'); throw createError('File content is required and must be a string', 400); }
  apiLogger.info('Starting report configuration import', { contentLength: fileContent.length });
  try {
    const result = await configImportService.importConfiguration(fileContent);
    res.json(result);
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : 'Unknown error';
    apiLogger.error('Import failed with exception', { error: errorMessage });
    if (errorMessage.includes('exceeds maximum allowed size')) throw createError(errorMessage, 413);
    else throw createError('Failed to import configuration', 500);
  }
}));
...
router.get('/:id/download', asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (fs.existsSync(reportSubDir) && fs.statSync(reportSubDir).isDirectory()) {
      ...
      reportFile = dirFiles.find(file => file.toLowerCase().endsWith('.pdf') || file.toLowerCase().endsWith('.docx'));
      if (reportFile) { filePath = path.join(reportSubDir, reportFile); apiLogger.debug('Report found in subdirectory', { id, reportFile, filePath }); }
    }

    if (!filePath) {
      const files = fs.readdirSync(reportsDir);
      reportFile = files.find(file => id && (file === id || file.startsWith(`${id}__`)));
      if (reportFile) { filePath = path.join(reportsDir, reportFile); apiLogger.debug('Report found in root directory (legacy)', { id, reportFile, filePath }); }
    }

    if (!reportFile || !filePath) { apiLogger.warn('Report file not found', { reportId: id }); throw createError('Report not found', 404); return; }
    if (!fs.existsSync(filePath)) { apiLogger.warn('Report file does not exist', { reportId: id, filePath }); throw createError('Report file not found', 404); }
    ...
    if (reportFile.includes('__')) downloadFileName = reportFile.substring(reportFile.indexOf('__') + 2);
    ...
    fileStream.on('error', (error) => {
      ...
      if (!res.headersSent) res.status(500).json({ success: false, message: 'Error downloading report', reportId: id });
    });
    fileStream.pipe(res);
    apiLogger.info('Report download started', { reportId: id, fileName: reportFile, fileSize: stats.size, contentType });
  } catch (error) {
    if (error instanceof Error && error.message.includes('not found')) throw error;
