...
describe('Property-Based Tests: PDF Report Redesign', () => {
  ...
  const reportConfigArb = fc.record({
    ...
    branding: fc.option(
      fc.record({
        companyName: fc.option(fc.constant('Acme Corp'), { nil: undefined }),
    ...
    it('should add footer with generation timestamp and page numbers to all pages', async () => {
      await fc.assert(
        fc.asyncProperty(reportDataArb, async (reportData) => {
          ...
          // Property 1.1: Footer must contain "Generated by OT Hub on"
          expect(text).toContain('Generated by OT Hub on');
          ...
          const timestampRegex = /Generated by OT Hub on ([^P]+)Page/g;
  ...
  describe('Property 2: Color Restriction', () => {
    ...
    it('should use custom company branding without colored backgrounds', async () => {
      await fc.assert(
        fc.asyncProperty(reportDataArb, async (reportData) => {
          const result = await reportService.generateReport(reportData);
          ...
          // Property: Report should contain the custom company branding
          expect(text).toContain('Acme Corp');

          // Property: Report should contain the app name subtitle
          expect(text).toContain('OT Hub');
        }),
        { numRuns: 10, timeout: 30000 }
      );
    });
    ...
    it('should have consistent generation timestamp across all pages', async () => {
      await fc.assert(
        fc.asyncProperty(reportDataArb, async (reportData) => {
          ...
          const timestampRegex = /Generated by OT Hub on/g;
